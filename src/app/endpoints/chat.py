...
async def translate_chat(request: GeminiRequest):
    ...
    try:
        ...
        response = await session_manager.get_response(
            request.model, request.message, request.files
        )
    ...
    if request.model:
        try:
            ...
            response = await gemini_client.generate_content(
                message=user_message, model=request.model.value, files=None
            )
            return convert_to_openai_format(
                response.text, request.model.value, is_stream
            )
        except Exception as e:
        ...
        raise HTTPException(
            status_code=400, detail="Model not specified in the request."
        )
